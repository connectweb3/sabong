    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (action === 'add') {
                ...
                alert(`Successfully added ₱${amount.toLocaleString()} to ${user.username}'s balance!`);
            } else {
                ...
                if (!adminData || adminData.credits < amount) {
                ...
                const { error: adminUpdateError } = await supabase
                    .from('profiles')
                    .update({
                        credits: adminData.credits - amount
                    })
                    .eq('id', adminId);
                ...
                alert(`Successfully transferred ₱${amount.toLocaleString()} to ${user.username}!`);
            }
                    ...
                    <div className="mb-4 p-4 bg-neutral-900/50 rounded-lg border border-neutral-700">
                        <div className="flex justify-between items-center">
                            ...
                            <div className="text-right">
                                ...
                                <p className="text-white font-bold font-mono">₱ {user.credits.toLocaleString()}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            ...
                            <p className="text-xs text-neutral-500 mt-2">
                                {action === 'add'
                                    ? 'Add credits without deducting from your balance'
                                    : 'Transfer credits from your balance to this user'}
                            </p>
                        </div>
                            ...
                            <div className="space-y-1">
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className="text-white font-mono font-bold">
                                        ₱ {(user.credits + amount).toLocaleString()}
                                    </span>
                                </div>
                            </div>
                        </div>
